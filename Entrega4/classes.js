...
class Floor extends Box {
	constructor(length, height, width, road) {
        ...
        var texture = new THREE.TextureLoader().load("toalha.jpg");
 		texture.wrapS = THREE.RepeatWrapping;
 		texture.wrapT = THREE.RepeatWrapping;
 		texture.repeat.set(4, 4);
        this.materialb = new THREE.MeshBasicMaterial({ wireframe: true, map: texture });
        this.materiall = new THREE.MeshLambertMaterial({ color: "rgb(50%, 50%, 50%)", emissive: "rgb(20%, 20%, 20%)", map: texture });
        this.materialp = new THREE.MeshPhongMaterial({ color: "rgb(50%, 50%, 50%)", emissive: "rgb(20%, 20%, 20%)", specular: "rgb(5%, 5%, 5%)", map: texture, shininess: 10 });
...
class Car extends Box {
    constructor(x, y, z) {
		...
		//-----------------------------Entrega4----------------------------------------
		this.spotlightEsquerda = new THREE.SpotLight(0xffffff, 2, 50, 0.37, 0, 8);
		this.spotlightEsquerda.position.set(0, 0.40, 0.15);
		this.spotlightEsquerda.target.position.set(-1, 0, 0.3);

		this.spotlightDireita = new THREE.SpotLight(0xffffff, 2, 50, 0.37, 0, 8);
		this.spotlightDireita.position.set(0, 0.40, -0.15);
		this.spotlightDireita.target.position.set(-1, 0, -0.3);

		this.add(this.spotlightEsquerda.target);
        this.add(this.spotlightEsquerda);
        this.add(this.spotlightDireita.target);
        this.add(this.spotlightDireita);
		//-----------------------------------//-----------------------------------------
    ...
    carandbutter() {
        this.translateX(-this.userData.velocity * 0.06);
		if(this.userData.velocity != 0) {
			this.userData.velocity = 0;
			this.userData.aceleration = 0;
